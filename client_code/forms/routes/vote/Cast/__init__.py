...
class Cast(CastTemplate):
    def __init__(self, **properties):
        self.item[
            "contract"
        ] = f"https://scrollexplorer.unifra.io/address/{globals.ballot_manager_address}"
        self.ballot = Ballot.get(key=self.url_dict["ballot_id"])
        self.selection = self.url_dict["selection"]
        self.proof = self.url_dict["proof"]
        self.encrypted_submission = nucypher.encrypt_vote(
            self.proof, self.selection, self.ballot.ends_at.timestamp()
        )
